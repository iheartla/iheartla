...
static Eigen::Matrix<double, 3, 1> dvec3_to_eigen(dvec3 &p){
	Eigen::Matrix<double, 3, 1> eigen_res;
	eigen_res << p[0], p[1], p[2];
	return eigen_res;
}

static Eigen::Matrix<double, 4, 4> xform_to_eigen(xform &R){
	Eigen::Matrix<double, 4, 4> res;
	// res << R[0], R[1], R[2], R[3],
	// R[4], R[5], R[6], R[7],
	// R[8], R[9], R[10], R[11],
	// R[12], R[13], R[14], R[15];
	res << R[0], R[4], R[8], R[12],
	R[1], R[5], R[9], R[13],
	R[2], R[6], R[10], R[14],
	R[3], R[7], R[11], R[15];
	return res;
...
static xform eigen_to_xform(Eigen::Matrix<double, 4, 4> &e){
	xform x(e(0, 0), e(1, 0), e(2, 0), e(3, 0),
...
static Eigen::Matrix<double, 4, 4> new_rot(double angle, Eigen::Matrix<double, 3, 1> p){
	dvec3 rot(p(0), p(1), p(2));
	xform R = xform::rot(angle, rot);
	return xform_to_eigen(R);
}

static Eigen::Matrix<double, 4, 4> new_trans(Eigen::Matrix<double, 3, 1> p){
	dvec3 rot(p(0), p(1), p(2));
	xform R = xform::trans(rot);
	return xform_to_eigen(R);
}
	...
	float lambda)
{
	...
	//           R * xform::trans(trans) * R *
	//           xform::trans(-centroid2);
	// std::cout<<"alignxf is :\n"<< xform_to_eigen(alignxf)<<std::endl;
	// Construct parameters
	dvec3 _centroid1(centroid1);
	Eigen::Matrix<double, 3, 1> eigen_centroid1 = dvec3_to_eigen(_centroid1); 
	dvec3 _centroid2(centroid2);
	Eigen::Matrix<double, 3, 1> eigen_centroid2 = dvec3_to_eigen(_centroid2);
	dvec3 _rot(rot);
	Eigen::Matrix<double, 3, 1> eigen_rot = dvec3_to_eigen(_rot); 
	dvec3 _trans(trans);
	Eigen::Matrix<double, 3, 1> eigen_trans = dvec3_to_eigen(_trans);
	//
	std::vector<Eigen::Matrix<double, 3, 1>> tmp;
	Eigen::Matrix<double, 3, 1>  t;
	Eigen::Matrix<double, 3, 3> RR;
	icp _icp(RR, eigen_rot, rotangle, 
		tmp, tmp, tmp, tmp, 
		eigen_trans, eigen_centroid1, eigen_centroid2, new_trans, new_rot);
	// std::cout<<"_icp.S is:\n"<<_icp.S<<std::endl; 
	alignxf = eigen_to_xform(_icp.S); 
	double delta = 0.0;
	for (size_t i = 0; i < npairs; i++) {
		dvec3 v1 = dvec3(pairs[i].p1);
		dvec3 v2 = dvec3(pairs[i].p2);
