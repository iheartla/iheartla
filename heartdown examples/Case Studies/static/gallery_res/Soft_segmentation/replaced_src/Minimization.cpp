...
double energy(std::vector<double> v, std::vector<cv::Vec3d> &means, std::vector<cv::Matx33d> &covs, bool sparse){
	...
	std::vector<Eigen::Matrix<double, 3, 1>> mu;
	std::vector<std::function<double(Eigen::Matrix<double, 3, 1>)>> mf;
	for(size_t i = 0; i < n; i++){
		...
		energy += alpha * dist;
		Eigen::Matrix<double, 3, 1> sm(c1, c2, c3);
		mu.push_back(sm);
		mf.push_back([i, means, covs](Eigen::Matrix<double, 3, 1> t)->double{
			cv::Vec3d nn = cv::Vec3d(t(0),t(1),t(2));
			double dist = pow(cv::Mahalanobis(nn, means.at(i), (covs.at(i)).inv()),2);
            return dist;
        });
	}
	if(sparse){ 
		...
		if(sum_squared == 0)
		{
			...
			soft _soft(mf, mu, constants::sigma, alphas);
			return _soft.F_S;
		}
	} else {
		sparsity = 0;
...
double d_maha(cv::Vec3d mean, cv::Matx33d cov, cv::Vec3d u, int i){ //the square has been removed from the mahalanobis distance so we don't need to deal with sqrt in derivative
	...
	cv::Vec3d df_m;
	double df_i;
	cv::Mat m = 2*cov_inv*(cv::Mat(u - mean));
	// Vec3f xyz((float*)m.data);
	// df_m = cv::Vec3d((float*)m.data);
	// df_i = df_m.val[i];
	df_i = m.data[i];
