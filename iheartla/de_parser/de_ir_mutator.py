...
class DIRTypeLift(DIRIterator):
    __instance = None

    @staticmethod
    def getInstance():
        if DIRTypeLift.__instance is None:
            DIRTypeLift()
        return DIRTypeLift.__instance

    def __init__(self):
        super().__init__()
        DIRTypeLift.__instance = self
...
class DIRMutator(DIRIterator):
    ...
    def init_type(self, type_walker, func_name):
        self.reset()
        super(DIRMutator, self).init_type(type_walker, func_name)
    ...
    def visit_equation(self, node, **kwargs):
        ...
        if node.eq_type == EqTypeEnum.DEFAULT:
            ...
            if res and len(res) > 0:
                ...
                assign_node = DIRTypeLift.getInstance().visit(assign_node)
                return assign_node
