...
class TypeWalker(NodeWalker):
                        ...
                        try:
                            ...
                            if rhs_id_node.la_type.is_function():
                                if lhs_id_node.contain_subscript():
                                    self.assert_expr(lhs_id_node.node_type == IRNodeType.SequenceIndex, self.get_err_msg_info(lhs_id_node.parseinfo, "Invalid assignment for function"))
    ...
    def extract_all_params(self, raw_param_list, **kwargs):
        ...
        if not self.pre_walk:
            for cur_set in self.dependency_set:
                self.assert_expr(cur_set in self.symtable, self.get_err_msg_info(self.dependency_dim_dict[cur_set], "Symbol {} not defined".format(cur_set)))
    ...
    def walk_WhereCondition(self, node, **kwargs):
        ...
        if node.index:
            ...
            self.assert_expr(type_node.la_type.is_integer_element(), self.get_err_msg_info(node.id[0].parseinfo, "Invalid index type: element must be integer"))
    ...
    def walk_FunctionType(self, node, **kwargs):
        ...
        if ret.is_vector():
            if isinstance(ret.rows, str):
                self.assert_expr(ret.rows in self.symtable or ret.rows in template_symbols, self.get_err_msg_info(ret_node.parse_info, "Vector as return value of function must have concrete dimension"))
                if ret.rows in template_symbols:
                    if ret.rows not in template_ret:
                        template_ret.append(ret.rows)
        elif ret.is_matrix():
            if isinstance(ret.rows, str):
                self.assert_expr(ret.rows in self.symtable or ret.rows in template_symbols, self.get_err_msg_info(ret_node.parse_info, "Matrix as return value of function must have concrete dimension"))
            if ret.rows in template_symbols:
                if ret.rows not in template_ret:
                    template_ret.append(ret.rows)
            if isinstance(ret.cols, str):
                self.assert_expr(ret.cols in self.symtable or ret.cols in template_symbols, self.get_err_msg_info(ret_node.parse_info, "Matrix as return value of function must have concrete dimension"))
    ...
    def walk_Import(self, node, **kwargs):
        ...
        self.assert_expr(node.package in self.packages, self.get_err_msg(self.get_line_info(node.parseinfo),
        ...
        for name in node.names:
            self.assert_expr(name in func_list, self.get_err_msg(self.get_line_info(node.parseinfo),
    ...
    def walk_AddSub(self, node, **kwargs):
        self.assert_expr(IF_COND in kwargs, self.get_err_msg(self.get_line_info(node.parseinfo),
    ...
    def walk_Assignment(self, node, **kwargs):
        ...
        if id0_info.ir.contain_subscript():
            ...
            for sub_index in range(len(left_subs)):
                ...
                self.assert_expr(sub_sym not in self.symtable, self.get_err_msg_info(node.left.right[sub_index].parseinfo, "Subscript has been defined"))
        ...
        if len(self.lhs_subs) > 0:
            for cur_index in range(len(self.lhs_subs)):
                ...
                if self.get_main_id(id0) not in self.symtable:
                    self.assert_expr(len(cur_sym_dict) > 0, self.get_err_msg_info(node.left.right[cur_index].parseinfo, "Subscript hasn't been used on rhs"))
                # self.check_sum_subs(self.lhs_subs[cur_index], cur_sym_dict)
                self.assert_expr(self.check_sum_subs(self.lhs_subs[cur_index], cur_sym_dict), self.get_err_msg_info(node.left.right[cur_index].parseinfo,
        ...
        if self.contain_subscript(id0):
            ...
            self.assert_expr('*' not in left_subs, self.get_err_msg_info(node.parseinfo, "LHS can't have * as subscript"))
            if node.op != '=':
                self.assert_expr(sequence in self.symtable, self.get_err_msg_info(id0_info.ir.parse_info,
            ...
            if len(left_subs) == 2:  # matrix
                if right_info.ir.node_type != IRNodeType.SparseMatrix:
                    self.assert_expr(sequence not in self.parameters, self.get_err_msg_info(id0_info.ir.parse_info, "{} is a parameter, can not be assigned".format(sequence)))
                if not (right_type.is_matrix() and right_type.sparse):
                    self.assert_expr(right_type.is_scalar(), self.get_err_msg_info(right_info.ir.parse_info, "RHS has to be scalar"))
            ...
            if node.op != '=':
                self.assert_expr(id0 in self.symtable, self.get_err_msg_info(id0_info.ir.parse_info, "{} hasn't been defined".format(id0)))
            else:
                if id0 in self.symtable:
                    err_msg = "{} has been assigned before".format(id0)
                    if id0 in self.parameters:
                        err_msg = "{} is a parameter, can not be assigned".format(id0)
                    self.assert_expr(False, self.get_err_msg_info(id0_info.ir.parse_info, err_msg))
    ...
    def walk_Summation(self, node, **kwargs):
        ...
        if node.cond:
            ...
            self.assert_expr(subs not in self.symtable, self.get_err_msg_info(sub_parse_info, "Subscript has been defined"))
        ...
        self.assert_expr(len(cur_sym_dict) > 0, self.get_err_msg_info(sub_parse_info, "Subscript hasn't been used in summation"))
        # self.check_sum_subs(subs, cur_sym_dict)
        self.assert_expr(self.check_sum_subs(subs, cur_sym_dict), self.get_err_msg_info(sub_parse_info, "Subscript has inconsistent dimensions"))
    ...
    def walk_Optimize(self, node, **kwargs):
        ...
        #
        self.assert_expr(exp_node.la_type.is_scalar(), self.get_err_msg_info(exp_node.parse_info, "Objective function must return a scalar"))
    ...
    def walk_Norm(self, node, **kwargs):
        ...
        if ir_node.value.la_type.is_scalar():
            self.assert_expr(node.single is not None, self.get_err_msg_info(node.parseinfo, "Norm error. Scalar type has to use | rather than ||"))
        elif ir_node.value.la_type.is_vector():
            self.assert_expr(node.single is None, self.get_err_msg_info(node.parseinfo, "Norm error. Vector type has to use || rather than |"))
            self.assert_expr(ir_node.norm_type != NormType.NormFrobenius and ir_node.norm_type != NormType.NormNuclear, self.get_err_msg(self.get_line_info(node.parseinfo),
            ...
            if ir_node.norm_type == NormType.NormIdentifier:
                self.assert_expr(ir_node.sub.la_type.is_matrix() or ir_node.sub.la_type.is_scalar(), self.get_err_msg(self.get_line_info(node.parseinfo),
                ...
                if ir_node.sub.la_type.is_matrix():
                    self.assert_expr(is_same_expr(ir_node.sub.la_type.rows, ir_node.sub.la_type.cols) and is_same_expr(ir_node.sub.la_type.rows, ir_node.value.la_type.rows), self.get_err_msg(self.get_line_info(node.parseinfo),
            ...
            if node.single:
                ...
                self.assert_expr(node.single is None, self.get_err_msg_info(node.parseinfo, "Norm error. MATRIX type has to use || rather than |"))
                self.assert_expr(ir_node.norm_type == NormType.NormFrobenius or ir_node.norm_type == NormType.NormNuclear, self.get_err_msg(self.get_line_info(node.parseinfo),
                ...
                if ir_node.norm_type == NormType.NormNuclear:
                    self.assert_expr(not ir_node.value.la_type.sparse, self.get_err_msg(self.get_line_info(node.parseinfo),
    ...
    def walk_InnerProduct(self, node, **kwargs):
        left_info = self.walk(node.left, **kwargs)
        right_info = self.walk(node.right, **kwargs)
        self.assert_expr(left_info.ir.la_type.is_vector(), self.get_err_msg_info(left_info.ir.parse_info, "Inner product error. Parameter {} must be vector".format(node.left.text)))
        self.assert_expr(right_info.ir.la_type.is_vector(), self.get_err_msg_info(right_info.ir.parse_info, "Inner product error. Parameter {} must be vector".format(node.right.text)))
        self.assert_expr(is_same_expr(left_info.ir.la_type.rows, right_info.ir.la_type.rows), self.get_err_msg_info(node.parseinfo,
    ...
    def walk_FroProduct(self, node, **kwargs):
        left_info = self.walk(node.left, **kwargs)
        right_info = self.walk(node.right, **kwargs)
        self.assert_expr(left_info.la_type.is_vector() or left_info.la_type.is_matrix(), self.get_err_msg_info(left_info.ir.parse_info, "Frobenius product error. Parameter {} must be vector or matrix".format(node.left.text)))
        self.assert_expr(right_info.la_type.is_vector() or right_info.la_type.is_matrix(), self.get_err_msg_info(right_info.ir.parse_info, "Frobenius product error. Parameter {} must be vector or matrix".format(node.right.text)))
    ...
    def walk_HadamardProduct(self, node, **kwargs):
        ...
        self.assert_expr(left_info.la_type.is_vector() or left_info.la_type.is_matrix(), self.get_err_msg_info(left_info.ir.parse_info, "Hadamard product error. Parameter {} must be vector or matrix".format(node.left.text)))
        self.assert_expr(right_info.la_type.is_vector() or right_info.la_type.is_matrix(), self.get_err_msg_info(right_info.ir.parse_info, "Hadamard product error. Parameter {} must be vector or matrix".format(node.right.text)))
        self.assert_expr(is_same_expr(left_info.la_type.rows, right_info.la_type.rows), self.get_err_msg_info(node.parseinfo,
        ...
        if left_info.la_type.is_matrix():
            self.assert_expr(is_same_expr(left_info.la_type.cols, right_info.la_type.cols), self.get_err_msg_info(node.parseinfo,
                                                                                        "Hadamard product error. Parameters {} and {} must have the same dimension".format(node.left.text, node.right.text)))
    ...
    def walk_CrossProduct(self, node, **kwargs):
        left_info = self.walk(node.left, **kwargs)
        right_info = self.walk(node.right, **kwargs)
        self.assert_expr(left_info.la_type.is_vector(), self.get_err_msg_info(left_info.ir.parse_info, "Cross product error. Parameter {} must be vector".format(node.left.text)))
        self.assert_expr(right_info.la_type.is_vector(), self.get_err_msg_info(right_info.ir.parse_info, "Cross product error. Parameter {} must be vector".format(node.right.text)))
        self.assert_expr(left_info.la_type.rows == 3, self.get_err_msg_info(left_info.ir.parse_info, "Cross product error. The dimension of parameter {} must be 3".format(node.left.text)))
        self.assert_expr(right_info.la_type.rows == 3, self.get_err_msg_info(right_info.ir.parse_info, "Cross product error. The dimension of parameter {} must be 3".format(node.right.text)))
    ...
    def walk_KroneckerProduct(self, node, **kwargs):
        ...
        self.assert_expr(left_info.la_type.is_vector() or left_info.la_type.is_matrix(), self.get_err_msg_info(left_info.ir.parse_info, "Kronecker product error. Parameter {} must be vector or matrix".format(node.left.text)))
        self.assert_expr(right_info.la_type.is_vector() or right_info.la_type.is_matrix(), self.get_err_msg_info(right_info.ir.parse_info, "Kronecker product error. Parameter {} must be vector or matrix".format(node.right.text)))
    ...
    def walk_DotProduct(self, node, **kwargs):
        ...
        self.assert_expr(left_info.la_type.is_vector(), self.get_err_msg_info(left_info.ir.parse_info, "Dot product error. Parameter {} must be vector".format(node.left.text)))
        self.assert_expr(right_info.la_type.is_vector(), self.get_err_msg_info(right_info.ir.parse_info, "Dot product error. Parameter {} must be vector".format(node.right.text)))
        self.assert_expr(is_same_expr(left_info.la_type.rows, right_info.la_type.rows), self.get_err_msg_info(node.parseinfo,
    ...
    def create_power_node(self, base, power):
        ...
        self.assert_expr(power.la_type.is_scalar(), self.get_err_msg_info(power.parse_info, "Power must be scalar"))
        power_node.la_type = ScalarType()
        self.assert_expr(base.la_type.is_scalar() or base.la_type.is_matrix(), self.get_err_msg_info(base.parse_info, "Base of power must be scalar or matrix"))
        if base.la_type.is_matrix():
            self.assert_expr(is_same_expr(base.la_type.rows, base.la_type.cols), self.get_err_msg_info(base.parse_info, "Power error. Rows must be the same as columns"))
    ...
    def walk_Power(self, node, **kwargs):
        ...
        if node.t:
            if 'T' in self.symtable:
                ...
                # transpose
                ir_node.t = node.t
                self.assert_expr(base_info.la_type.is_matrix() or base_info.la_type.is_vector(), self.get_err_msg_info(base_info.ir.parse_info,"Transpose error. The base must be a matrix or vecotr"))
            ...
            if base_info.la_type.is_matrix():
                self.assert_expr(base_info.la_type.is_matrix(), self.get_err_msg_info(base_info.ir.parse_info,"Inverse matrix error. The base must be a matrix"))
                self.assert_expr(is_same_expr(base_info.la_type.rows, base_info.la_type.cols), self.get_err_msg_info(base_info.ir.parse_info,"Inverse matrix error. The rows should be the same as the columns"))
                ir_node.la_type = MatrixType(rows=base_info.la_type.rows, cols=base_info.la_type.rows, sparse=base_info.la_type.sparse)
            else:
                self.assert_expr(base_info.la_type.is_scalar(), self.get_err_msg_info(base_info.ir.parse_info,
    ...
    def walk_Solver(self, node, **kwargs):
        ...
        self.assert_expr(left_info.la_type.is_matrix(), self.get_err_msg_info(left_info.ir.parse_info, "Parameter {} must be a matrix".format(node.left.text)))
        self.assert_expr(right_info.la_type.is_matrix() or right_info.la_type.is_vector(), self.get_err_msg_info(left_info.ir.parse_info, "Parameter {} must be a matrix or vector".format(node.right.text)))
        node_type = None
        if left_info.la_type.is_matrix():
            self.assert_expr(is_same_expr(left_info.la_type.rows, right_info.la_type.rows), self.get_err_msg_info(left_info.ir.parse_info, "Parameters {} and {} should have the same rows".format(node.left.text, node.right.text)))
    ...
    def walk_Transpose(self, node, **kwargs):
        ...
        ir_node.f = f_info.ir
        self.assert_expr(f_info.la_type.is_matrix() or f_info.la_type.is_vector(), self.get_err_msg_info(f_info.ir.parse_info,"Transpose error. The base must be a matrix or vector"))
    ...
    def walk_Squareroot(self, node, **kwargs):
        ...
        f_info = self.walk(node.f, **kwargs)
        self.assert_expr(f_info.la_type.is_scalar(), self.get_err_msg_info(f_info.ir.parse_info, "Squareroot error. The base must be a scalar"))
    ...
    def walk_Function(self, node, **kwargs):
        if isinstance(node.name, str):
            if contains_sub_symbol(node.name):
                ...
                if split_res[-1].isnumeric():
                    ...
                    self.assert_expr(split_res[-1] in self.symtable, self.get_err_msg_info(node.parseinfo, "Subscript not defined"))
                    self.assert_expr(self.symtable[split_res[-1]].is_int_scalar(), self.get_err_msg_info(node.parseinfo, "Subscript has to be integer"))
        ...
        if name_type.is_function():
            ...
            param_list = []
            self.assert_expr(len(node.params) == len(name_type.params), self.get_err_msg_info(node.parseinfo, "Function error. Parameters count mismatch"))
            symbols = set()
            for index in range(len(node.params)):
                ...
                if len(name_type.template_symbols) == 0:
                    self.assert_expr(name_type.params[index].is_same_type(param_info.ir.la_type), self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
                    continue
                if name_type.params[index].is_scalar():
                    self.assert_expr(name_type.params[index].is_same_type(param_info.ir.la_type), self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
                elif name_type.params[index].is_vector():
                    self.assert_expr(param_info.ir.la_type.is_vector(), self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
                    if name_type.params[index].rows in name_type.template_symbols:
                        ...
                        self.assert_expr(is_same_expr(name_type.params[index].rows, param_info.ir.la_type.rows), self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
                elif name_type.params[index].is_matrix():
                    self.assert_expr(param_info.ir.la_type.is_matrix(), self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
                    if name_type.params[index].rows in name_type.template_symbols:
                        convertion_dict[name_type.params[index].rows] = param_info.ir.la_type.rows
                    else:
                        self.assert_expr(is_same_expr(name_type.params[index].rows, param_info.ir.la_type.rows), self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
                    ...
                    if name_type.params[index].cols in name_type.template_symbols:
                        ...
                        self.assert_expr(is_same_expr(name_type.params[index].cols, param_info.ir.la_type.cols), self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
                elif name_type.params[index].is_set():
                    self.assert_expr(param_info.ir.la_type.is_set(), self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
                    self.assert_expr(name_type.params[index].size == param_info.ir.la_type.size, self.get_err_msg_info(param_info.ir.parse_info, "Function error. Parameter type mismatch"))
            ...
            self.assert_expr(len(node.params) == 1, "Not a function")  # never reach
    ...
    def walk_IfCondition(self, node, **kwargs):
        ...
        if add_sub_node is not None:
            ...
            self.assert_expr(add_sub_node.get_child(IRNodeType.AddSub) is None, self.get_err_msg_info(add_sub_node.parse_info, "Multiple +- symbols in a single expression"))
    ...
    def walk_InCondition(self, node, **kwargs):
        ...
        for item in node.left:
            ...
            self.assert_expr(item_info.la_type.is_scalar(), self.get_err_msg_info(item_info.ir.parse_info, "Must be scalar type"))
        ir_node.items = item_node
        set_info = self.walk(node.right, **kwargs)
        ir_node.set = set_info.ir
        self.assert_expr(set_info.la_type.is_set(), self.get_err_msg_info(set_info.ir.parse_info, "Must be set type"))
        self.assert_expr(len(item_node) == set_info.la_type.size, self.get_err_msg_info(node.parseinfo, "Size doesn't match for set"))
    ...
    def walk_NeCondition(self, node, **kwargs):
        left_info = self.walk(node.left, **kwargs)
        right_info = self.walk(node.right, **kwargs)
        self.assert_expr(left_info.la_type.is_same_type(right_info.la_type), self.get_err_msg(self.get_line_info(node.parseinfo),
                                                              self.get_line_info(node.parseinfo).text.find(node.op),
    ...
    def walk_GreaterCondition(self, node, **kwargs):
        left_info = self.walk(node.left, **kwargs)
        right_info = self.walk(node.right, **kwargs)
        self.assert_expr(left_info.la_type.is_scalar(), self.get_err_msg_info(left_info.ir.parse_info, "LHS has to be scalar type for comparison"))
        self.assert_expr(right_info.la_type.is_scalar(), self.get_err_msg_info(right_info.ir.parse_info, "RHS has to be scalar type for comparison"))
    ...
    def walk_IdentifierSubscript(self, node, **kwargs):
            ...
            for right in node.right:
                if right != '*':
                    ...
                    if not self.visiting_lhs and not isinstance(r_content, int):
                        self.assert_expr(r_content in self.symtable, self.get_err_msg_info(right.parseinfo, "Subscript has not been defined"))
                else:
        ...
        if node.right[0] == '*' and len(node.right) > 1:
            ...
            while cur < len(raw_text):
                if raw_text[cur] == ',':
                    ...
                    self.assert_expr(raw_text[cur] == ' ', self.get_err_msg(line_info, line_info.text.index('_*') + 1, "* must be used with ," ))
                cur += 1
        right = []
        left_info = self.walk(node.left, **kwargs)
        if not self.is_param_block:
            if left_info.content == 'I' and 'I' not in self.symtable:
                ...
                self.assert_expr(len(right_sym_list) == 1 and '*' not in right_sym_list, self.get_err_msg_info(left_info.ir.parse_info,
                                                                                        "Invalid identity matrix"))
                ir_node = NumMatrixNode(parse_info=node.parseinfo)
                if isinstance(right_sym_list[0], str):
                    self.assert_expr(right_sym_list[0] in self.symtable, self.get_err_msg(line_info, line_info.text.index('_{}'.format(right_sym_list[0])) + 1,"{} unknown".format(right_sym_list[0])))
                ...
                self.assert_expr(left_info.content in self.symtable, self.get_err_msg_info(left_info.ir.parse_info,
                ...
                if self.symtable[left_info.content].is_sequence():
                    ...
                    ir_node.main_index = main_index_info.ir
                    if len(node.right) == 1:
                        self.assert_expr(node.right[0] != '*', self.get_err_msg(self.get_line_info(node.parseinfo),
                                                                  self.get_line_info(node.parseinfo).text.find('*'),
                                                                  "Can't use * as the index for a sequence"))
                        la_type = self.symtable[left_info.content].element_type
                    elif len(node.right) == 2:
                        self.assert_expr(self.symtable[left_info.content].is_vector_seq(), self.get_err_msg_info(left_info.ir.parse_info,
                                                                       "Two subscripts are used for sequence of vector"))
                        self.assert_expr('*' not in node.right, self.get_err_msg(self.get_line_info(node.parseinfo),
                                                                  self.get_line_info(node.parseinfo).text.find('*'),
                                                                  "* can't be used here"))
                        main_index_info = self.walk(node.right[0])
                        ...
                        self.assert_expr(self.symtable[left_info.content].is_matrix_seq(), self.get_err_msg_info(left_info.ir.parse_info,
                        ...
                        if '*' in node.right:
                            ...
                            if node.right[1] == '*':
                                self.assert_expr(node.right[2] != '*', self.get_err_msg(self.get_line_info(node.parseinfo),
                                                                  self.get_line_info(node.parseinfo).text.find('*'),
                    ...
                    self.assert_expr(len(node.right) == 2, self.get_err_msg_info(left_info.ir.parse_info,
                    ...
                    if '*' in node.right:
                        if node.right[0] == '*':
                            self.assert_expr(node.right[1] != '*', self.get_err_msg(self.get_line_info(node.parseinfo),
                                                                  self.get_line_info(node.parseinfo).text.find('*'),
                                                                  "Only one * is allowed as subscripts"))
                    ...
                    self.assert_expr(len(node.right) == 1, self.get_err_msg_info(left_info.ir.parse_info,
                                                                                        "Only one subscript is allowed"))
                    self.assert_expr(node.right[0] != '*', self.get_err_msg(self.get_line_info(node.parseinfo),
                                                                  self.get_line_info(node.parseinfo).text.find('*'),
                                                                  "Subscript can't be *"))
                    index_info = self.walk(node.right[0])
                    self.assert_expr(index_info.la_type.is_scalar(), self.get_err_msg_info(index_info.ir.parse_info,
                    ...
                    if id0 != 'I':  # special case
                        ...
                        if 'I' not in self.symtable:
                            if not la_is_inside_matrix(**kwargs):
                                self.assert_expr(id0_info.ir.contain_subscript(), self.get_err_msg_info(id0_info.ir.parse_info,
    ...
    def walk_SparseMatrix(self, node, **kwargs):
        ...
        if op == '=':  # require dims
            ...
            id_name = new_id
            self.assert_expr(all_ids[0] in self.symtable, self.get_err_msg_info(node.parseinfo, "Sparse matrix: need dim"))
            ...
            if not isinstance(id1, int):
                self.assert_expr(id1 in self.dim_dict or id1 in self.parameters, self.get_err_msg_info(node.parseinfo, "Sparse matrix: dim {} is not defined".format(id1)))
            id2 = self.symtable[all_ids[0]].cols
            if not isinstance(id2, int):
                self.assert_expr(id2 in self.dim_dict or id2 in self.parameters, self.get_err_msg_info(node.parseinfo, "Sparse matrix: dim {} is not defined".format(id2)))
            la_type = MatrixType(rows=id1, cols=id2, sparse=True, index_var=index_var, value_var=value_var)
            self.symtable[new_id] = la_type
        elif op == '+=':
            self.assert_expr(all_ids[0] in self.symtable, self.get_err_msg_info(node.parseinfo, "{} is not defined".format(all_ids[0])))
            la_type = self.symtable[all_ids[0]]
            id_name = all_ids[0]
            id1 = self.symtable[all_ids[0]].rows
            if not isinstance(id1, int):
                self.assert_expr(id1 in self.symtable, self.get_err_msg_info(node.parseinfo,
                                                                   "Sparse matrix: dim {} is not defined".format(id1)))
            id2 = self.symtable[all_ids[0]].cols
            if not isinstance(id2, int):
                self.assert_expr(id2 in self.symtable, self.get_err_msg_info(node.parseinfo,
    ...
    def walk_Vector(self, node, **kwargs):
        ...
        for exp in node.exp:
            ...
            self.assert_expr(exp_info.la_type.is_scalar() or exp_info.la_type.is_vector(), self.get_err_msg_info(node.parseinfo, "Item in vector must be scalar or vector"))
    ...
    def walk_Matrix(self, node, **kwargs):
        ...
        if block:
            ...
            self.assert_expr(valid, self.get_err_msg_info(node.parseinfo,  "Block matrix error. Invalid dimensions"))
            rows = real_dims[0]
            cols = real_dims[1]
            if len(undef_list) > 0:
                ...
                for i, j in undef_list:
                    ...
                    if 'I' in cur_ir.raw_text:
                        ...
                        if not (type_array[i][j].rows == 1 and type_array[i][j].cols == 1):
                            self.assert_expr('0' == cur_ir.raw_text or '1' == cur_ir.raw_text, self.get_err_msg_info(cur_ir.parse_info, "Can not lift {}".format(cur_ir.raw_text)))
    ...
    def walk_NumMatrix(self, node, **kwargs):
        ...
        if isinstance(id1, str):
            self.assert_expr(id1 in self.symtable, self.get_err_msg_info(id1_info.ir.parse_info, "{} unknown".format(id1)))
        ir_node.left = node.left
        if node.left == '0':
            self.assert_expr(la_is_inside_matrix(**kwargs), self.get_err_msg_info(node.parseinfo, "Zero matrix can only be used inside matrix"))
        if node.id2:
            id2_info = self.walk(node.id2, **kwargs)
            ir_node.id2 = id2_info.ir
            id2 = id2_info.content
            if isinstance(id2, str):
                self.assert_expr(id2 in self.symtable, self.get_err_msg_info(id2_info.ir.parse_info, "{} unknown".format(id2)))
    ...
    def create_math_node_info(self, func_type, param_info, remains=[]):
        ...
        if MathFuncType.MathFuncInvalid < func_type < MathFuncType.MathFuncAtan2:
            ...
            self.assert_expr(param.la_type.is_scalar(), self.get_err_msg_info(param.parse_info, "Parameter must be scalar type"))
            for par_info in remains:
                par = par_info.ir
                remain_list.append(par)
                self.assert_expr(par.la_type.is_scalar(), self.get_err_msg_info(par.parse_info, "Parameter must be scalar type"))
                symbols = symbols.union(par_info.symbols)
        elif func_type == MathFuncType.MathFuncTrace:
            self.assert_expr(param.la_type.is_matrix() and param.la_type.rows == param.la_type.cols, self.get_err_msg_info(param.parse_info, "Parameter must be valid matrix type"))
            ret_type = ScalarType()
        elif func_type == MathFuncType.MathFuncDiag:
            self.assert_expr((param.la_type.is_matrix() and param.la_type.rows == param.la_type.cols) or param.la_type.is_vector(), self.get_err_msg_info(param.parse_info, "Parameter must be valid matrix or vector type"))
            ...
            self.assert_expr(param.la_type.is_matrix(), self.get_err_msg_info(param.parse_info, "Parameter must be valid matrix type"))
            ...
            self.assert_expr(param.la_type.is_matrix(), self.get_err_msg_info(param.parse_info,
    ...
    def create_trig_node_info(self, func_type, param_info, power):
        ...
        param = param_info.ir
        if power:
            self.assert_expr(param.la_type.is_scalar(), self.get_err_msg_info(param.parse_info, "Parameter must be scalar type for the power"))
    ...
    def walk_ArithFactor(self, node, **kwargs):
        ...
        if node.id0:
            ...
            if type(node.id0).__name__ == "IdentifierSubscript":
                self.dyn_dim = True
                self.assert_expr(len(node.id0.right) == 1 and node.id0.right[0] != '*', self.get_err_msg_info(node.id0.parseinfo, "Invalid dimension"))
    ...
    def handle_identifier(self, identifier, ir_node, id_node):
        ...
        if self.contain_subscript(identifier):
            arr = self.get_all_ids(identifier)
            new_var_name = None
            self.assert_expr(len(arr[1]) == 1, self.get_err_msg_info(ir_node.parse_info, "Parameter {} can't use multiple subscripts".format(arr[0])))
            val = arr[1][0]  # subscript
            self.assert_expr(not val.isnumeric(), self.get_err_msg_info(id_node.parse_info, "Parameter {} can't have constant subscript".format(arr[0])))
            ...
            self.assert_expr(arr[0] not in self.symtable, self.get_err_msg_info(id_node.parse_info, "Parameter {} has been defined.".format(arr[0])))
            if id_type.is_vector():
                if id_type.is_dynamic_row():
                    if id_node.id1.is_node(IRNodeType.SeqDimIndex):
                        self.assert_expr(id_node.id1.main_index.get_name() == val, self.get_err_msg_info(id_node.id1.parse_info, "Dimension {} has different subscript".format(id_node.id1.main.get_name())))
                        ...
                        if id_node.id1.get_main_id() in self.symtable:
                            self.assert_expr(self.symtable[id_node.id1.get_main_id()].is_same_type(row_seq_type), self.get_err_msg_info(id_node.id1.parse_info,
                ...
                if id_type.is_dynamic_row():
                    if id_node.id1.is_node(IRNodeType.SeqDimIndex):
                        ...
                        if id_node.id1.get_main_id() in self.symtable:
                            self.assert_expr(self.symtable[id_node.id1.get_main_id()].is_same_type(
                ...
                if id_type.is_dynamic_col():
                    if id_node.id2.is_node(IRNodeType.SeqDimIndex):
                        self.assert_expr(id_node.id1.main_index.get_name() == val, self.get_err_msg_info(id_node.id2.parse_info, "Dimension {} has different subscript".format(id_node.id2.main.get_name())))
                        ...
                        if id_node.id2.get_main_id() in self.symtable:
                            self.assert_expr(self.symtable[id_node.id2.get_main_id()].is_same_type(
            ...
            self.assert_expr(identifier not in self.symtable, self.get_err_msg_info(id_node.parse_info, "Parameter {} has been defined.".format(identifier)))
